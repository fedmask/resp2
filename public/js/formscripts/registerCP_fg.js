...
/*Add validation rule that checks CF validity (e.g. 16 characters length and the 16th character validation)*/

jQuery.validator.addMethod("checkCF", function(value, element) {
...
/*Add validation rule that checks phone number validity*/

jQuery.validator.addMethod("checkPhone", function(value, element) {
    
		var a = value;
		var filter = /^[0-9-+]+$/;
		if (filter.test(a) && (a.length>8)) {
			return true;
		}
		else {
			return false;
...
/*Add validation rule that checks user's legal age*/
...
jQuery.validator.addMethod("checkAge", function(value, element) {
		...
		actualDate.setFullYear(actualDate.getFullYear() - legalAge);
        
		if ((actualDate - userDate) < 0){
            return false;
        }
        return true;
	}, "Spiacente, non hai 18 anni compiuti.");


/*
1)Set typeahead autocomplet plugin's settings (e.g. find matches function).
2)Set datepicker settings (e.g. localization and visual setting)
*/


function getSpecializz(){
	$.post("formscripts/checkRegister.php",
	{
		getSpecializz:	1
	},
	function(json_specializz){
		var specializz = JSON.parse(json_specializz);
		for	(var index = 0; index < specializz.length; index++) {
			$("#tipoSpecializz").append('<option val="'+specializz[index]+'">'+specializz[index]+'</option>');
		}			
	});
}
...
$(document).ready(function () {
	...
	/*richiedo le tipologie di attività degli operatori sanitari*/
	getSpecializz();
	/*Set form Css to avoid empty spaces and overflow*/
	$("#example-advanced-form .content").css({"min-height":"100px", "height": "auto"});
	$("#example-advanced-form .content > fieldset").css({"position": "relative"});
...
var citiesSuggest = new Bloodhound({
  ...
  local: cities
	...
	{
	  name: 'cities',
	  source: citiesSuggest,
	  limit: 8000
	});

	$('#livingCity').typeahead({
	  hint: true,
	  highlight: true,
	  minLength: 1
	},
...
/*Handle blood type dropdown event*/

	$('ul.dropdown-menu li a').click(function (e) {
		$('#bloodTypeDrop').html(e.target.id + '<span class="caret"></span>');
		$('#bloodTypeDrop').val(e.target.id);
		$('#bloodType').val(e.target.id);		
	});

/*Hide bloodType input (it's a hidden input able to get dropdown value)	*/
	$('#bloodType').hide();


	
/*Settings for fileInput plugin*/	
	$("#profilePic").fileinput({
		language: "it",
		overwriteInitial: false,
		maxFileSize: 2000,
		uploadAsync: true,
		uploadUrl: 'formscripts/uploadProfilePic.php',
		multiple: false,
	...
	/*Hide input for profile picture*/
	$('#profilePicHidden').hide();
	
	/*On profile picture uploaded save the image's path in the hidden input*/	
	$('#profilePic').on('fileuploaded', function(event, data, previewId, index) {
		$('#profilePicHidden').val(data.response.success);
        //console.log('File uploaded triggered target: ' + $('#profilePicHidden').val());
    });
});


/*Show registration form and set jQuery steps parameters*/
...
form.steps({
    ...
    onStepChanging: function (event, currentIndex, newIndex)
    {
		...
		/*Hide profilePicHidden input (it's a hidden input able to get profile picture's path)	*/
		//if(currentIndex == 2)
			//$('profilePicHidden').hide();
    ...
    onFinished: function (event, currentIndex)
    {
	    ...
	    errorPlacement: function errorPlacement(error, element) { error.appendTo(element.parents(".form-group"))},
	    errorClass: 'help-block col-lg-4',
        errorElement: 'div',
    ...
    rules: {
		...
		userName:{
				...
				"remote":
						{
							url: 'formscripts/verifyUsernameExists.php',
							type: "post"
						}
				},
		password: {
		...
		email: {
			...
			/*"remote":
                    {
						url: 'formscripts/verifyMailExists.php',
						type: "post"
					}
			*/
		...
		CF: {
			...
			"remote":
					{
						url: 'formscripts/verifyCFExists.php',
						type: "post"
					}
		},
		...
		email:
                {
                    ...
                    remote: jQuery.validator.format('L\'indirizzo email immesso è già stato utilizzato.')
                },
		CF:
				{
					remote: jQuery.validator.format('Il codice fiscale immesso è già stato utilizzato.')
				},
		userName:	{
						remote: jQuery.validator.format('Lo username immesso è già stato utilizzato.')
					}
	}
});